...
import src.models.REINFORCE_PPO as rl
...
env = mao.ClusteringEnv(p_job_arrival=0.4, observation_mode='image',
...
num_job_sets = 500
batch_size = 50
model = rl.model(sess, state_dim, action_dim, lr, network_widths=[50, 50, 40], update_step=10, epsilon=0.1)
    ...
    for current_job_cnt in range(num_job_sets):
        ...
        for current_ep in range(batch_size):
            ...
            for ep_len in range(max_ep_size):
                ...
                ep_reward += r
                s = s2
                if done or (ep_len == max_ep_size - 1):
